...
class WoZaiXiaoYuan():
    ...
    def get_token_info(self):
        ...
        def set_txtpad_text(i):
            ...
            token = lib.get_txtpad_text(
                self.user_info[i]["txtpad_name"], 'token')
            date = lib.get_txtpad_text(
                self.user_info[i]["txtpad_name"], 'token_date')
            # 第一个文本 令牌
    ...
    def punch_card(self, punch_card_api, punch_card_info, token, index):
        ...
        '''
        # 检查口令状态 手动的自己填写日期
        token_statue = self.get_token_dead_time(
            self.user_info[index]["token_date"])
        if token_statue == -1:
            # 发送过期警告
            token_statue = str(
            ...
            # 发送将过期警告
            token_statue += '\n' + \
                str(self.user_info[index]["name"] +
                    "的口令:" + self.headers["token"])
            send_warn(token_statue)
            lib.log("即将过期")
        # 设置口令，开始打卡
...
def main():
    ...
    '''
    如果是调用一日三检打卡
    wzxy.heat_data["seq"] = wzxy.get_seq()
    if wzxy.heat_data["seq"] == 0:
        return 0
    '''
    # 循环为每个用户打卡
    for index in range(len(wzxy.user_info)):
        ...
        '''
        如果是调用一日三检打卡
        wzxy.heat_data["temperature"] = wzxy.get_random_temprature()
        wzxy.punch_card(wzxy.heat_url, wzxy.heat_data,
                        wzxy.user_info[index]["token"], index)
                        '''
